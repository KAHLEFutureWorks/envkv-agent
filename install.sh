...
for tree in backend spike tools docs extension; do
  rm -rf "${TARGET:?}/$tree"
  [ -d "$PACKAGE_DIR/$tree" ] && cp -a "$PACKAGE_DIR/$tree" "$TARGET/"
done

for file in Dockerfile compose.yaml .dockerignore .env.example README.md install.sh VERSION; do
  [ -f "$PACKAGE_DIR/$file" ] && cp -a "$PACKAGE_DIR/$file" "$TARGET/"
done
...
[ "$health" = '{"status":"ok"}' ] || die "Unerwartete Antwort der Statusabfrage: $health"
...
[ "$code" = "401" ] || die "Der Zugriffsschutz greift nicht. Erwartet 401, erhalten $code."
